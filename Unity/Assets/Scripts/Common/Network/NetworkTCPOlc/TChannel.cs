...
#region 网络线程
		...
		public TChannel(long id, IPEndPoint ipEndPoint, TService service)
		{
			...
			this.Service.SynchronizationContextSO.PostNext(this.ConnectAsync);
		}
		...
		public TChannel(long id, Socket socket, TService service)
		{
			...
			// 下一帧再开始读写
			this.Service.SynchronizationContextSO.PostNext(() =>
			{
				this.StartRecv();
				this.StartSend();
			});
